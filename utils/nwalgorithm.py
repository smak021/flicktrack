...
# New Algorithm. Making db rows lesser by focusing more on theatre rather than shows.
#  Two methods. one is to use find and only count next upcoming show instead of counting all show.
#  But pre tracking is not possible with this method. Second one is to count all data. 
def main_data(film_namee,film_ID, fm_loc, loc_slug, venue):
    ...
    try:
        ...
        values = soup.find('a',{'data-session-id':True,'data-venue-code':venue},class_='showtime-pill')
    except :
        print("Error reading")
        values = False
    total_seat=0
